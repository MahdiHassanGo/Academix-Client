...
const Total = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const usersResponse = await fetch('https://b10a12-server-side-mahdi-hassan-go.vercel.app/users/count');
        const usersCount = await usersResponse.json();
        console.log('Total Users:', usersCount);

        let teachersCount = { total: 0 }; 
        try {
          const teachersResponse = await fetch('https://b10a12-server-side-mahdi-hassan-go.vercel.app/teachers/count');
        ...
        const enrollmentsResponse = await fetch('https://b10a12-server-side-mahdi-hassan-go.vercel.app/enrollments/count');
