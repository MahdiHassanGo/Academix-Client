...
const MyEnrolledClassDetails = () => {
  ...
  useEffect(() => {
    const fetchClassDetails = async () => {
      try {
        ...
        const classResponse = await fetch(`https://server-ecru-nu-72.vercel.app/classes/${classId}`);
        if (!classResponse.ok) {
        ...
        const assignmentsResponse = await fetch(`https://server-ecru-nu-72.vercel.app/assignments/${classId}`);
  ...
  const handleSubmitAssignment = async (assignmentId) => {
    try {
      const response = await fetch(`https://server-ecru-nu-72.vercel.app/assignments/${assignmentId}/submit`, {
        method: "POST",
        headers: {
